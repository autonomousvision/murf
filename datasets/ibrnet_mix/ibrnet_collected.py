...
class IBRNetCollectedDataset(Dataset):
    def __init__(self, root_dir, split='train', n_views=3, random_crop=True,
                 no_random_view=False,
                 large_subsample=False,
                 include_more_scenes=False,
                 **kwargs):
        rootdir = root_dir
        mode = split
        ...
        self.folder_path1 = os.path.join(rootdir, 'ibrnet_collected_1/')
        self.folder_path2 = os.path.join(rootdir, 'ibrnet_collected_2/')
        ...
        all_scenes = glob.glob(self.folder_path1 + '*') + \
            glob.glob(self.folder_path2 + '*')

        if include_more_scenes:
            self.folder_path3 = os.path.join(rootdir, 'ibrnet_collected_more/')
            all_scenes = all_scenes + \
                sorted(glob.glob(self.folder_path3 + '*'))
    ...
    def __getitem__(self, idx):
        ...
        world_center = (render_pose.dot(
            np.array([[0, 0, mean_depth, 1]]).T)).flatten()[:3]
        ...
        if self.mode == 'train':
            ...
            if self.large_subsample:
                subsample_factor = np.random.choice(np.arange(5, 11))
            else:
                subsample_factor = np.random.choice(
                    np.arange(1, 4), p=[0.2, 0.45, 0.35])

            if self.no_random_view:
        ...
        for id in nearest_pose_ids:
            ...
            if self.rectify_inplane_rotation:
                train_pose, src_rgb = rectify_inplane_rotation(
                    train_pose, render_pose, src_rgb)
