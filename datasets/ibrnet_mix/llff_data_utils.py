...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True):
    ...
    if poses.shape[-1] != len(imgfiles):
        ...
        imgfiles = [os.path.join(imgdir, f) for f in sorted(
            os.listdir(imgdir)) if f[0:-4] in imnames]
...
def load_llff_data(basedir, factor=8, recenter=True, bd_factor=.75,
    ...
    out = _load_data(basedir, factor=factor, load_imgs=load_imgs)
    if out is None:
        return
    else:
    ...
    if subsample_factor is not None:
        ...
        bds = bds[:, ::subsample_factor]
        imgfiles = imgfiles[::subsample_factor]
