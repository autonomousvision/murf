...
def view_obj2camera_rgb(view):
    ...
    ref_camera = np.concatenate(
        [list(rgb.shape[:2]), intrinsics_4x4.flatten(), c2w.flatten()])
...
def sample_target_view_for_training(views, input_rig_id, input_ids):
    ...
    input_cam_positions = np.array(
        [input_rig_views[i].camera.w_f_c[:3, 3] for i in input_ids])
    ...
    selected_view = views[remaining_rig_ids[selected_id]
                          ][remaining_cam_ids[selected_id]]
...
def get_all_views_in_scene_cam_path(all_views):
    ...
    for rig in all_views:
        for i in range(len(rig)):
            image_path, img_size, intrinsics_4x4, c2w = view_obj2camera_rgb_path(
                rig[i])
...
def sort_nearby_views_by_angle(query_pose, ref_poses):
    ...
    ref_directions = ref_directions / \
        np.linalg.norm(ref_directions, axis=-1, keepdims=True)
...
def _IntrinsicsFromViewDict(view_params):
    ...
    intrinsics[1, 1] = (view_params['focal_length'] *
                        view_params['pixel_aspect_ratio'])
...
def ReadScene(base_dir):
    ...
    for views in model_json:
        all_views.append([ReadView(base_dir, view_json)
...
class SpacesFreeDataset(Dataset):
    def __init__(self, root_dir, split='train', n_views=3, is_debug=False,
        ...
        self.folder_path = os.path.join(rootdir, 'spaces_dataset/data/800/')
        self.mode = mode
        ...
        if is_debug:
            ...
            train_scene_ids = [i for i in np.arange(
                0, 100) if i not in eval_scene_ids]
        if mode == 'train':
            self.scene_dirs = [os.path.join(
                self.folder_path, 'scene_{:03d}'.format(i)) for i in train_scene_ids]
        else:
            self.scene_dirs = [os.path.join(
                self.folder_path, 'scene_{:03d}'.format(i)) for i in eval_scene_ids]
        ...
        for scene_dir in self.scene_dirs:
            ...
            rgb_paths, img_sizes, intrinsicss, c2w_mats = get_all_views_in_scene_cam_path(
                views)
    ...
    def __getitem__(self, idx):
        ...
        selected_rig_id = np.random.randint(
            low=0, high=num_rigs)  # select a rig position
        ...
        if self.mode == 'train' and self.random_crop:
            render_rgb, render_camera, ref_rgbs_np, ref_cameras = random_crop(render_rgb, render_camera,
                                                                              ...
                                                                              size=(320, 640), spaces_dataset=True)
        ...
        for rgb_tmp in ref_rgbs_np:
            if render_rgb.shape[0] != rgb_tmp.shape[0] or render_rgb.shape[1] != rgb_tmp.shape[1]:
                print('unmatched image shape on spaces dataset, skip')
                return self.__getitem__(0)

        sample['images'] = torch.stack([self.transform(img) for img in [
                                       *[x for x in ref_rgbs_np], render_rgb]]).float()  # (V, C, H, W)
        # ibrnet camera format: [(h, w, intr(16), extr(16))]
        sample['extrinsics'] = np.stack([np.linalg.inv(
            x[-16:].reshape(4, 4)) for x in [*ref_cameras, render_camera]]).astype(np.float32)  # (V, 4, 4)
        sample['intrinsics'] = np.stack([x[2:-16].reshape(4, 4)[:3, :3] for x in [
                                        *ref_cameras, render_camera]]).astype(np.float32)  # (V, 3, 3)
        sample['view_ids'] = np.array(
            [*nearby_view_ids_selected, selected_rig_id])
        sample['scene'] = f"{self.get_name()}_{all_rgb_paths[0].split('/')[-3]}"
        sample['img_wh'] = np.array(
            [render_camera[1], render_camera[0]]).astype('int')
