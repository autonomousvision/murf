...
def rectify_inplane_rotation(src_pose, tar_pose, src_img, th=40):
    ...
    rotated = cv2.warpAffine(src_img, M, (w, h), borderValue=(
        255, 255, 255), flags=cv2.INTER_LANCZOS4)
    rotated = rotated.astype(np.float32) / 255.
    return out_pose, rotated


def random_crop(rgb, camera, src_rgbs, src_cameras, size=(400, 600), center=None, spaces_dataset=False):
    h, w = rgb.shape[:2]
    ...
    if center is not None:
        ...
        if spaces_dataset:
            center_h = np.random.randint(
                low=out_h // 2 + 1, high=h - out_h // 2 - 16)
        else:
            center_h = np.random.randint(
                low=out_h // 2 + 1, high=h - out_h // 2 - 1)
        center_w = np.random.randint(
            low=out_w // 2 + 1, high=w - out_w // 2 - 1)

    rgb_out = rgb[center_h - out_h // 2:center_h + out_h //
                  2, center_w - out_w // 2:center_w + out_w // 2, :]
