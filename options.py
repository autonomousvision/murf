...
def set(opt_cmd={}, load_confd=True):
    ...
    if load_confd and os.path.isfile("configs/confidentiality.yaml"):
        opt_conf = load_options("configs/confidentiality.yaml")
        opt_base = override_options(opt_base, opt_conf, key_stack=[], safe_check=False)
    # override with command line arguments
    # opt = override_options(opt_base,opt_cmd,key_stack=[],safe_check=True)
    # no safe check
    opt = override_options(opt_base,opt_cmd,key_stack=[],safe_check=False)
    process_options(opt)
    log.options(opt)
    return opt
...
def process_options(opt):
    ...
    if hasattr(opt, "data_test"):
        for k, v in getattr(opt, "data_test").items():
            if v is None:
                continue
            if getattr(v, "scene_list", None) is not None and isinstance(getattr(v, "scene_list", None), str):
                attr_str = getattr(v, "scene_list")
                setattr(v, "scene_list", attr_str.split(','))

    # update gpu_ids list
    if isinstance(getattr(opt, "gpu_ids"), str):
        gpu_ids = getattr(opt, "gpu_ids")
        setattr(opt, "gpu_ids", [int(x) for x in gpu_ids.split(',') if x.strip()])
