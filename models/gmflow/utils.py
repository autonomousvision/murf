...
def sample_features_by_grid(raw_whole_feats, grid, align_corners=True, mode='bilinear', padding_mode='border',
                            local_radius=0, local_dilation=1, average_cosine=False, concat_cosine=False):
    if local_radius <= 0:
    ...
    if average_cosine or concat_cosine:
        # concat on the batch dim for further processing after computing cosine
        sampled_feats = sampled_feats.permute(0, 3, 1, 2).reshape(b * local_h * local_w, c, n_rays, n_pts)  # [B*(2R+1)^2, C, N, D]
    else:
        sampled_feats = sampled_feats.reshape(b, c*n, local_h, local_w)  # [B, C*n_rays*n_pts, 2R+1, 2R+1]
        # TODO: may need to compare with average per-pixel cosine, instead of average feature first and then compute cosine
        avg_feats = F.adaptive_avg_pool2d(sampled_feats, (1, 1))  # [B, C*n_rays*n_pts, 1, 1]
        sampled_feats = avg_feats.reshape(b, c, n_rays, n_pts)

    return sampled_feats



def merge_patches_to_channel(feature,
                  patch_size=2,
                  ):
    
    # feature: [B, C, H, W, D]
    b, c, h, w, d = feature.size()
    assert h % patch_size == 0 and w % patch_size == 0
    feature = feature.view(b, c, patch_size, h // patch_size, patch_size, w // patch_size, d
                           ).permute(0, 1, 2, 4, 3, 5, 6).reshape(
        b, c * patch_size * patch_size, h // patch_size, w // patch_size, d)  # [B, C*P*P, H/P, W/P, D]
    return feature



def reverse_channel_patches(feature, 
                            patch_size=2):
    # feature: [B, C*P*P, H/P, W/P, D]
    b, c, h, w, d = feature.size()
    assert c % (patch_size ** 2) == 0

    feature = feature.view(b, c // (patch_size ** 2), patch_size, patch_size, h, w, d
                           ).permute(0, 1, 2, 4, 3, 5, 6
                                     ).reshape(b, c // (patch_size ** 2), patch_size * h, patch_size * w, d)  # [B, C, H, W, D]
