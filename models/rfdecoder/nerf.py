...
class NeRF(torch.nn.Module):
    ...
    # [B,...,3]
    def forward(self, opt, points_3D, ray_unit=None, mode=None, **kwargs):
        if opt.decoder.posenc:
            points_enc = self.positional_encoding(
                opt, points_3D, L=opt.decoder.posenc.L_3D)
            points_enc = torch.cat(
                [points_3D, points_enc], dim=-1)  # [B,...,6L+3]
        else:
        ...
        for li, layer in enumerate(self.mlp_feat):
            ...
            if li == len(self.mlp_feat)-1:
                ...
                if opt.nerf.density_noise_reg and mode == "train":
                    density += torch.randn_like(density) * \
                        opt.nerf.density_noise_reg
                # relu_,abs_,sigmoid_,exp_....
                density_activ = getattr(torch_F, opt.decoder.density_activ)
        ...
        if opt.nerf.view_dep:
            ...
            if opt.decoder.posenc:
                ray_enc = self.positional_encoding(
                    opt, ray_unit, L=opt.decoder.posenc.L_view)
                ray_enc = torch.cat([ray_unit, ray_enc],
                                    dim=-1)  # [B,...,6L+3]
            else:
                ray_enc = ray_unit
    ...
    def composite(self, opt, ray, rgb_samples, density_samples, depth_samples, setbg_opaque,
        ...
        if render_depth_no_boundary is not None:
            remove = render_depth_no_boundary
            depth_prob = (prob[:, :, remove:-remove]) / (
                torch.sum(prob[:, :, remove:-remove], dim=2, keepdim=True) + 1e-8)
            depth = (depth_samples[:, :, remove:-remove]
                     * depth_prob).sum(dim=2)  # [B,HW,1]
        else:
            depth_prob = prob / (torch.sum(prob, dim=2, keepdim=True) + 1e-8)
            depth = (depth_samples*depth_prob).sum(dim=2)  # [B,HW,1]
